...
app.MapGet("/api/profile", [Authorize] ([FromServices] IUserService userService, ClaimsPrincipal claimsPrincipal) => userService.Profile(claimsPrincipal.UserName()));

app.MapGet("/api/users", [Authorize(Roles = "Admin")] async ([FromServices]IUserService userService) => await userService.ToListAsync(0, 10));
...
app.MapPost("/api/factors", [Authorize(Roles = "Admin")] async ([FromServices] IFactorService factorService, int orderId, long customerId, DateTime dueDate) 
    => factorService.Create(orderId, customerId, dueDate));

app.MapPost("/api/pay", [Authorize] async ([FromServices] IFactorService factorService, ClaimsPrincipal claimsPrincipal, Guid invoiceId) 
